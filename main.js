...
function isEmailCorrect() {
    ...
    if (email.value == "") {
        ...
        error[6].style.display = "block";
        email.value = null;  //이메일 지워지게 함
        email.focus();            //이메일 지워지게 함
    } else {
        error[6].style.display = "none";
    }
}
...
function  checkPhoneNum(){
    let isPhonNum = /^([01]{2})([01679]{1})([0-9]{3,4})([0-9]{4})$/;
if(mobile.value ==""){
    error[7].innerHTML="필수정보입니다."
    error[7].style.display="block";

}else if(!isPhonNum.test(mobile.value)){
    error[7].innerHTML="형식에 맞지 않은 번호입니다."
    error[7].style.display="block";
}else{
